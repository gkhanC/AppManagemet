...
namespace AppManagement
{
    class HelperProcess
    {
        ...
        public List<AppManagementModel> AppProcessList()
        {
            ...
            {
                //Görev çubuğunda listelenen programların MainWindowsTitle alanı dolu oluyor.
                //Skype aktif değilken mainwindowsTitle boş oluyor.Bu yüzden ProcessName bakılıp listeye alınıyor.
                //Dropbox gibi programlarda notify icon olup arkada çalışıyor.Bu yüzden ProcessName bakılıp listeye alınıyor.
                if (!String.IsNullOrEmpty(item.MainWindowTitle)
                    || item.ProcessName == "Skype"
                    || item.ProcessName == "Dropbox")
                {
                    //Bu programı çalışan uygulamalar arasında listememesi için;
        ...
        public void killProcess(string _processName)
        {
            if (!string.IsNullOrWhiteSpace(_processName))
            {
                ...
                {
                    //Bazı programlar aynı isimde 1'den fazla exe açıyor.
                    //Ana exe'yi ProcessName'den yakalayamadğımız için, yetkimiz olmayan bir exe'yi kapatmak istediğimizde,
                    //Program cath'e düşüyor. Doğru exe'yi yakalayıp kapatmak için tekrar geri dönüyoruz.
                    return;
                }
            }
        }
